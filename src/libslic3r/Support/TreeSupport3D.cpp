    ...
    const bool                   mergelayer)
{
    ...
    if (settings.move) {
        ...
        if (overspeed > 0) {
            ...
            // Orca: safe_movement_distance is used as the safe_offset_inc() step, so keep it
            // non-zero to preserve branch movement with zero-clearance support settings.
    ...
    std::function<void()>                throw_on_cancel)
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, merging_areas.size(), 1),
        ...
        for (size_t merging_area_idx = range.begin(); merging_area_idx < range.end(); ++ merging_area_idx) {
            ...
            // Orca: safe_movement_distance is used as a divisor and as the safe_offset_inc() step,
