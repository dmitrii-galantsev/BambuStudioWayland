...
namespace Slic3r {
namespace GUI {
const int                  c_move_cube_id                        = 1;
const std::array<float, 4> SVG_Move_GrabberColor                 = {1.0, 1.0, 0.0, 1.0};
const std::array<float, 4> SVG_Move_GrabberHoverColor            = {0.7, 0.7, 0.0, 1.0};

// TRN - Title in Undo/Redo stack after rotate with SVG around emboss axe
const std::string rotation_snapshot_name = "SVG rotate";
// NOTE: Translation is made in "m_parent.do_rotate()"

// TRN - Title in Undo/Redo stack after move with SVG along emboss axe - From surface
const std::string move_snapshot_name = "SVG move";
...
std::string choose_svg_file()
{
    ...
    if (input_files.size() != 1) BOOST_LOG_TRIVIAL(warning) << "SVG file dialog result contain multiple files but only first is used.";
...
EmbossShape select_shape(std::string_view filepath, double tesselation_tolerance = get_tesselation_tolerance(1.))
{
    EmbossShape shape;
    ...
    if (filepath.empty()) {
        ...
        if (svg.path.empty()) return {}; // file was not selected
    } else {
...
std::string GLGizmoSVG::get_tooltip() const
{
    return "";
}
...
void GLGizmoSVG::on_enable_grabber(unsigned int id) {
    m_rotate_gizmo.enable_grabber(0);
}

void GLGizmoSVG::on_disable_grabber(unsigned int id) {
    m_rotate_gizmo.disable_grabber(0);
}

Emboss::DataBasePtr GLGizmoSVG::create_emboss_data_base(std::shared_ptr<std::atomic<bool>> &cancel, ModelVolumeType volume_type, std::string_view filepath)
{
    ...
    auto result        = std::make_unique<Emboss::DataBase>(name, cancel, std::move(shape));
    result->is_outside = volume_type == ModelVolumeType::MODEL_PART;
    return result;
}

Emboss::CreateVolumeParams GLGizmoSVG::create_input(GLCanvas3D &canvas, ModelVolumeType volume_type)
{
    auto            gizmo     = static_cast<unsigned char>(GLGizmosManager::Svg);
    const GLVolume *gl_volume = canvas.get_selection().get_first_volume();//modify by bbs //get_first_hovered_gl_volume(canvas);
    Plater *        plater    = wxGetApp().plater();
    auto            register_mesh_pick = [this]() { register_single_mesh_pick(); };
#ifndef EXECUTE_UPDATE_ON_MAIN_THREAD
    return Emboss::CreateVolumeParams{canvas, plater->get_camera(),  plater->build_volume(), plater->get_ui_job_worker(),
                                      register_mesh_pick, m_raycast_manager, m_raycast_condition,volume_type,gizmo,gl_volume};
#else
    return Emboss::CreateVolumeParams{canvas, plater->get_camera(), plater->build_volume(), volume_type, gizmo, gl_volume};
 #endif
}
...
const IconManager::Icon &get_icon(const IconManager::VIcons &icons, IconType type, IconState state) { return *icons[(unsigned) type][(unsigned) state]; }
...
std::string GLGizmoSVG::on_get_name() const
{
    return "SVG";
}
