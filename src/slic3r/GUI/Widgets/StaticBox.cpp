    ...
    if (background_color2.count() == 0) {
        if ((border_width && border_color.count() > 0) || background_color.count() > 0) {
            ...
            if (border_width && border_color.count() > 0) {
                ...
                // Orca: snap rect edges to physical pixel boundaries so the 1px pen
                // doesn't straddle a pixel boundary on scaled (HiDPI) displays
                // (fixes glitches around TextInput/ComboBox after scrolling).
                auto snap = [&](double logical) -> int {
                    return (int)(ceil(logical * scale) / scale);
                };

                int deflate = snap(border_width / 2.0);  // at 175%: snap(0.5) = ceil(0.875)/1.75 = 1/1.75 -> 1
                rc.x      += deflate;
                rc.y      += deflate;
                rc.width  -= deflate * 2;
                rc.height -= deflate * 2;
